/*
 * ht16k33.cpp - used to talk to the htk1633 chip to do things like turn on LEDs
 or scan keys
 ...
 *             Created using https://www.arduino.cc/en/Hacking/LibraryTutorial
 and ht16k33 datasheet
...
#define HT16K33_SS_STANDBY                                                     \
    B00000000                       // System setup - oscillator in standby mode
...
void HT16K33::begin(uint8_t address) {
    ...
    i2c_write(HT16K33_DSP | HT16K33_DSP_ON |
              HT16K33_DSP_NOBLINK);           // Display on and no blinking
...
// NOTE: Don't send odd amount of data if using LSB, then it will send one to
// much
//
uint8_t HT16K33::i2c_write(uint8_t cmd, uint8_t *data, uint8_t size,
                           boolean LSB) {
    uint8_t i;
    Wire.beginTransmission(_address);
    Wire.write(cmd);
...
/****************************************************************/
// internal function - read a byte from specific address (send one byte(address
// to read) and read a byte)
//
...
void HT16K33::define16segFont(uint16_t *ptr) {
    ...
    Serial.println();
    Serial.print("DEBUG2: ");
    Serial.println((uint16_t)_seg16Font, HEX);
    Serial.println();
    Serial.print("DEBUG3: ");
    Serial.println(ptr[0], HEX);
    Serial.println(ptr[1], HEX);
    Serial.println(ptr[2], HEX);
    Serial.println();
    Serial.print("DEBUG4: ");
    Serial.println(_seg16Font[0], HEX);
...
/****************************************************************/
// Send the display ram info to chip - kind of commit all changes to the outside
// world
//
...
// To do it on chip a call to "sendLed" is needed
//
uint8_t HT16K33::set7Seg(uint8_t dig, uint8_t cha,
                         boolean dp) { // position 0-15, 0-15 (0-F
    // Hexadecimal), decimal point on|off
    if (cha >= 0 && cha < 16 && dig >= 0 && dig < 16) {
        if (dig >= 0 && dig <= 7) {
            dig = dig * 2;
        } else {
            dig = ((dig - 8) * 2) + 1;
        } // re-arrange digit positions
        uint8_t num = _seg7Font[cha];
        if (dp) {
            bitSet(num, (7));
        } else {
            bitClear(num, (7));
        } // Set decimal point on 7th bit
        displayRam[dig] = num;
        return 0;
    } else {
        return 1;
...
//
uint8_t
HT16K33::set16Seg(uint8_t dig,
                  uint8_t cha) { // position 0-15, 0-15 (0-F Hexadecimal)
    if (cha >= 0 && cha < 128 && dig >= 0 && dig < 8) {
        dig = dig * 2;
        // reverse the bits using a lookup table
        //    displayRam[dig+1] =
        //    BitReverseTable256[lowByte(seg16Chartable[cha])];
        //    displayRam[dig] =
        //    BitReverseTable256[highByte(seg16Chartable[cha])];
...
//    Serial.print(lowByte(_seg16Font[cha]),HEX);Serial.print(F("
//    "));Serial.println(highByte(&_seg16Font[cha]),HEX);
#ifdef PSDEBUG
        Serial.print((uint16_t)_seg16Font, HEX);
        Serial.print(F(":"));
        Serial.print(_seg16Font[cha] & 0xff, HEX);
        Serial.print(F(" "));
        Serial.println(_seg16Font[cha] >> 8, HEX);
#endif
        return 0;
    } else {
        return 1;
...
//
uint8_t HT16K33::keyINTflag() { return i2c_read(HT16K33_IFAP); } // keyINTflag
...
#ifdef __GNUC__
uint16_t _popcount(uint16_t x) { return __builtin_popcount(x); }
#else
...
uint8_t HT16K33::keysPressed() {
    ...
    return (_popcount(_keyram[0]) + _popcount(_keyram[1]) +
            _popcount(_keyram[2]));
} // keysPressed
...
void HT16K33::_updateKeyram() {
    ...
    _keyram[0] = curkeyram[1] << 8 |
                 curkeyram[0]; // datasheet page 21, 41H is high 40H is low
...
int8_t HT16K33::readKey(boolean clear) {
    ...
    for (i = 0; i < 3; i++) {
        diff = _keyram[i] ^
               oldKeyData[i]; // XOR old and new, any changed bit is set.
    ...
    return 0; // apperently no new key was pressed - old might still be held
              // down,
              // pass clear=true to see it
} // readKey
